...
# @Last modified time: 2020-07-16T11:15:54-05:00
import os
import sys
...
inputs=[23,'CoMPt_master_v3_L.csv','CoMPt_master_v3_R.csv',1,70,1,18,0.258*100000*0.012043]
#inputs=[3,'Harm.out']
...
if ((len(inputs)==1) and (inputs[0]=='--usage')):
    ...
    print('Arguments: [Master file L] [Master Files R] [1st PL] [final PL] [1st zlayer] [final zlayer] [scaling]')
    ...
    if mode==1:
        ...
        if len(inputs)<2:
            ...
            filel=inputs[1]
            filer=inputs[2]
            pls=[int(inputs[3]),int(inputs[4])]
            zl=[int(inputs[5]),int(inputs[6])]
            if len(inputs)<8:
        ...
        if len(inputs)<2:
            ...
            if len(inputs)<8:
                ...
                scl=float(inputs[7])
                print('Scaling torques by '+str(scl))
            harm_fit(filel,filer,pls,zl,scl)
            print('Harmonics out to file Harm.out')

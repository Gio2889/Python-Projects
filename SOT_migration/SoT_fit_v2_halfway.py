...
# @Last modified time: 2020-07-10T14:31:31-05:00
import os
import sys
...
###############################################################################
##################### make array of psimr and philmr ##########################
lmarr=np.array([])
w1=(5/(7*6))*np.pi
w2=(9/(7*10))*np.pi
wangles=angles[['th','phi']].copy()
wangles['i']=wangles.index+1
...
for l in lm:
    for i in range(0,32):
        if i==0:
            psphar=[np.array([psilmr(l[0],l[1],wangles.iloc[i,0],wangles.iloc[i,1]),philmr(l[0],l[1],wangles.iloc[i,0],wangles.iloc[i,1])])]
        else:
            next=[np.array([psilmr(l[0],l[1],wangles.iloc[i,0],wangles.iloc[i,1]),philmr(l[0],l[1],wangles.iloc[i,0],wangles.iloc[i,1])])]
            psphar=np.concatenate((psphar,next))
    if l==lm[0]:
        lmarr=[psphar]
    else:
        nlm=[psphar]
        lmarr=np.concatenate((lmarr,nlm))

lmarr[0]
###########################################################################
############ The index lm goes 1-4 and its defined in the block above######
############ refers to the tuple of [l,m]  ################################
def VSHproj(ds,type,lm):
    ...
    if type<3:
        ...
        res=0
        for i in range(32):
            print(lmarr[lm-1][i][type-1])
            res=res+wangles.iloc[i,3]*vf.iloc[i].dot(lmarr[lm-1][i][type-1])
        return res
    else:
...
def fitTrq(set,set2,m,*args):
    ...
    VSHsym =[VSHproj(fsurfsets,1,1),VSHproj(fsurfsets,2,2), VSHproj(fsurfsets,1,3),VSHproj(fsurfsets,2,4)]
    VSHas =[VSHproj(fsurfsets,2,1),VSHproj(fsurfsets,1,2), VSHproj(fsurfsets,2,3),VSHproj(fsurfsets,1,4)]
    return VSHsym,VSHas
...
if mode==1:
    ...
    fileh=inputs[0]
    scl=inputs[1]
    ds=pd.read_csv(fileh,sep=' ',names=['PL','A1','A2','A3','A4','B1','B2','B3','B4'])
    ds=ds[['A1','A2','A3','A4','B1','B2','B3','B4']].mean().to_frame().transpose()
    print(ds)
else:

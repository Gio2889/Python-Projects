...
# @Last modified time: 2020-12-23T15:43:47-06:00
import os
import sys
...
def harm_fit(ds1,ds2,pls,zl,mag):
    ...
    for a in range(1,33):
        for p in range(pls[0],pls[-1]+1):
            lp=maxpl+1-p
            cpl=dsl.loc[((dsl['PL']==lp) & (dsl['angle']==a))][zl[0]-1:zl[-1]][['tx','ty','tz']].sum().to_frame().transpose()
            cpl['PL']=p
...
#inputs=[23,'CoMPt_master',1,70,1,18]
inputs=[3,'Harm.out',2.0]
...
if ((len(inputs)==1) and (inputs[0]=='--usage')):
    ...
    print('Basic use is "python SoT_fit_v2.py [mode] [arguments]"')
    ...
    print('Arguments: [Master file name] [1st PL] [final PL] [1st zlayer] [final zlayer] [scaling]')
    print('Mode 3: Calculates SOT coefficients averaged over the number of PL in a "Harm.out" file')
    print('Arguments: ["Harm.out" file] [scaling](optional) ')
    ...
    if mode==1:
        ...
        if (len(inputs[1:]))%3!=0:
            ...
            print('use "python SoT_fit_v2.py --usage" to see specifics')
            exit()
        inputls=[]
        ...
        if len(inputs)<2:
            ...
            filename=inputs[1]
            filel=filename+'_L.csv'
            filer=filename+'_R.csv'
        ...
        if len(inputs)<2:
            ...
            if len(inputs)>3:
                ...
                df=pd.read_csv(fileh,names=['A1','A2','A3','A4','B1','B2','B3','B4'],delimiter=' ')
                df.iloc[0:20]
                df=df.iloc[spl:fpl]
            else:

...
def harm_fit(ds1,ds2,pls,zl,mag):
    ...
    if prnt==1:
        for a in range(1,33):
            cpl=dsl.loc[(dsl['angle']==a)][['tx','ty','tz']].sum().to_frame().transpose()/(pls[-1])
            cpr=dsr.loc[(dsr['angle']==a)][['tx','ty','tz']].sum().to_frame().transpose()/(pls[-1])
            cpl['angle']=a
            cpr['angle']=a
            if a==1:
                dffl=cpl
                ...
                dffl=dffl.append(cpl)
                dffr=dffr.append(cpr)
        lsftl=dffl.rename(columns={'angle':'i'})
        lsftr=dffr.rename(columns={'angle':'i'})
        res=list(chain.from_iterable(list(fitTrq(lsftl,lsftr,mag))))
        for x in res:
            if x==res[-1]:
                print(x)
            else:
...
# MODE: 3 Averages
##############################################################################
################# this variable controls output to file or console############
################# prnt = 0 prints to Harm.out file ###########################
################# prnt = 1 prints to console #################################
prnt=1
##############################################################################
...
inputs=[2,'M19_Adis_25dis_70_2x_71221',1,70,1,19]
#inputs=[3,'Harm.out',1,70]
...
if ((len(inputs)==1) and (inputs[0]=='--usage')):
    ...
    if mode==1:
        ...
        print('Calculating vector spherical harmonics from master files')
        if len(inputs)<2:
            print('Not enough arguments provided.')
            print('use "python SoT_fit_v6.py --usage" to see specifics')
            exit()
        elif len(inputs)>8:
